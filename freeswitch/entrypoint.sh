...
password_file="/var/run/freeswitch-secrets/password"
...
# Set password from file if it exists
if [ -f "$password_file" ]; then
  password=$(cat "$password_file")
  sed -i -e "s/name=\"password\" value=\"JambonzR0ck$\"/name=\"password\" value=\"$password\"/g" /usr/local/freeswitch/conf/autoload_configs/event_socket.conf.xml
fi
